...
int fmt_format(const char *format, char *buf, size_t buf_size, size_t max_args, va_list args) {
   ...
   int fmt_format(const char *format, char *buf, size_t buf_size, size_t max_args, va_list args) {
       ...
       for (const char *p = format; *p != '\0'; p++) {
           ...
           // --- Parse length modifier ---
           // Supported modifiers: "l", "ll", "h", and now "z"
           int length = 0;  // 0: default, 1: 'l', 2: 'll', -1: 'h', 3: 'z'
           if (*p == 'l') {
               length = 1;
               p++;
               if (*p == 'l') {
           ...
           switch (spec) {
               case 'd': {
                   ...
                   long long val;
                   if (length == 3) {
                       /* For %zd: assume the argument is of type ssize_t */
                       #ifdef _WIN32
                       /* Windows does not define ssize_t by default; use __int64 or similar if needed */
                       val = (long long)va_arg(args, ptrdiff_t);
                       #else
               ...
               case 'u': {
                   ...
                   unsigned long long val;
                   if (length == 3) {
                       /* For %zu: assume the argument is of type size_t */
               ...
               case 'X': {
                   ...
                   unsigned long long val;
                   if (length == 3) {
                       /* For %zx or %zX: assume the argument is of type size_t */
                       val = va_arg(args, size_t);
                   } else if (length == 2) {
                       val = va_arg(args, unsigned long long);
                   } else if (length == 1) {
